...
class RFBProxy(object):
    ...
    def feed(self, data):
        ...
        while self._length <= len(self._curbuf):
            ...
            (self._length, self._state) = self._state(x) #返回并套用self._state函数
        return
    ...
    def start_2(self, name):
        ...
        for e in self.preferred_encoding:
            #add by luoxiao : self.send(pack('>l', e))
            if self.debug:
                print >>sys.stderr, 'send encoding: e=%d' % e
            #end add
            self.send(pack('>L', e))
    ...
    def frameend(self):
        self.sink.flush(self.time()) # add by luoxiao ：视频帧更新完毕，写入flv文件。
        return self.loop()
    ...
    def framerect_1(self, data):
        ...
        #add by luoxiao
        if self.debug:
            print >>sys.stderr, 'framerect_1: data=%r, %d x %d at (%d,%d), enc=%d' % (data,width, height, x, y, enc)
        #end add
        self.rectpos = (x, y)
        ...
        #add by luoxiao, 服务器端分辨率更新消息
        elif enc == 0xFFFFFF21:
            if self.debug:
                print >>sys.stderr, 'NewFBSize:%d,%d' % (width, height)
            self.sink.onNewFBSize(x, y, width, height) #通知分辨率有变化
            return  self.framerect()  
        #add end
    ...
    def encraw_1(self, data):
        ...
        #edit by luoxiao, 使得vnc实际分辨率小于启动参数-C中指定的录像分辨率时图像居中
        #self.sink.update_screen_rgbabits(self.rectpos, self.rectsize, data) #将图像写入flvscreen缓冲
        
        (width,height) = self.sink.screensize #启动参数-C中指定的录像高度和宽度
        (x,y) = self.rectpos
        self.sink.update_screen_rgbabits((x,y), self.rectsize, data) #将图像写入flvscreen缓冲
        #end edit
        return self.framerect()
