/*
  ...
  add by luoxiao : text format - "UFT-8 not BOM" 
*/
...
FlvScreen_init(PyFlvScreen* self, PyObject* args, PyObject* kwargs)
{
    ...
    self->blk_size = blk_size;     // default = 32
    self->blk_width = blk_width;   // 1024/32 = 32
    self->blk_height = blk_height; //  768/32 = 24
    self->blocks = PyMem_Malloc(blk_width * blk_height); //32*24，block count
    if (self->blocks == NULL) return -1;
    self->tmppix = PyMem_Malloc(blk_size * blk_size * sizeof(FLVPixel));//32*32*FLVPixel
    if (self->tmppix == NULL) return -1;
    memset(self->blocks, 1, blk_width * blk_height);//self->blocks, set 1
    self->pix_width = blk_width * blk_size;//32*32
    self->pix_height = blk_height * blk_size;//24*32
    self->pixels = PyMem_Malloc(self->pix_width * self->pix_height * sizeof(RGBAPixel));//1024*768*RGBAPixel
    if (self->pixels == NULL) return -1;
    memset(self->pixels, 0, self->pix_width * self->pix_height * sizeof(RGBAPixel));//self->pixels, set 0
    return 0;
}
 ...
 *   copy the rgba data from vncdata to self->pixels.
 */
static PyObject*
FlvScreen_blit_rgba(PyFlvScreen* self, PyObject* args)
{
    ...
    /* copy the image data */
    {
	RGBAPixel* src = (RGBAPixel*)PyString_AsString(data); // 来自vnc的像素指针
...
//edit by luoxiao
//static PyObject*
//FlvScreen_reset(PyFlvScreen* self, PyObject* args) 
//{
...
static PyMethodDef FlvScreen_methods[] = {
	...
	{ "reset", (PyCFunction)FlvScreen_reset, METH_VARARGS,
      "reset"
    },
